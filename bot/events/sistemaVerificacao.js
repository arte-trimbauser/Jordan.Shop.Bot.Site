...
const CONFIG = {
    CANAL_VERIFICACAO_ID: config.CANAL_VERIFICACAO_ID,
    CANAL_LOGS_ID: config.LOGS_CHANNEL_ID,
    CARGO_NAO_VERIFICADO_ID: config.CARGO_NAO_VERIFICADO_ID,
    CARGO_VERIFICADO_ID: config.CARGO_VERIFICADO_ID,
...
async function handleVerificacaoInteraction(interaction, client) {
    ...
    if (customId === 'iniciar_verificacao') {
        ...
        if (member.roles.cache.has(CONFIG.CARGO_VERIFICADO_ID)) {
            return interaction.reply({ content: 'Ja estas verificado!', flags: MessageFlags.Ephemeral });
        }
