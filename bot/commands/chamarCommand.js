...
const STAFF_CHAMAR_ROLES = config.STAFF_ROLES;
...
async function registrarComandoChamar(client) {
    try {
        ...
        if (guildId) {
            ...
            console.log('✅ Comando /chamar registado:', result.name);
        } else {
            console.log('⚠️ GUILD_ID não definido, a registar globalmente');
...
async function handleChamarCommand(interaction, client) {
    ...
    if (!isStaff) {
        return interaction.editReply({ content: '❌ **Acesso Negado!** Apenas Staff pode usar este comando.' });
    }

    if (!channel.name.startsWith('ticket-') && !channel.name.startsWith('staff-')) {
        return interaction.editReply({ content: '❌ Este comando só pode ser usado em canais de ticket!' });
    }
    ...
    if (!topic) {
        return interaction.editReply({ content: '❌ Não foi possível identificar o cliente deste ticket!' });
    }

    const clienteId = topic.split('|')[0];
    if (!clienteId || !/^\d{17,19}$/.test(clienteId)) {
        return interaction.editReply({ content: '❌ ID do cliente inválido no tópico do canal!' });
    }
    ...
    try {
        ...
        const embedSaiu = new EmbedBuilder()
            ...
            .setDescription(`O cliente <@${clienteId}> **não está mais presente** neste ticket.\n\n🔒 Podes fechar o ticket se desejares.`)
            .setColor('#ff0000')
            .setTimestamp();
        const row = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId(`fechar_ticket_saida_${channel.id}`).setLabel('🔒 Fechar Ticket').setStyle(ButtonStyle.Danger)
        );
        return interaction.editReply({ embeds: [embedSaiu], components: [row] });
    }
    ...
    try {
        const embedDM = new EmbedBuilder()
            ...
            .setDescription(`O staff **${user.username}** chamou-te em ${channel}`)
            .setTimestamp();
        const rowDM = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setLabel('Ir para o Ticket').setURL(`https://discord.com/channels/${guild.id}/${channel.id}`).setStyle(ButtonStyle.Link)
        );
        await clienteMember.send({ embeds: [embedDM], components: [rowDM] });
    } catch (err) {
        return interaction.editReply({ content: `❌ Não foi possível chamar **${clienteMember.user.username}** (DMs fechadas).` });
    }

    await channel.send(`📢 <@${clienteMember.id}> (${clienteMember.user.username}), foste solicitado aqui por <@${user.id}> (${user.username})!`);
    ...
    setImmediate(async () => {
        try {
            const logChannel = await guild.channels.fetch(config.TICKET_LOGS_ID).catch(() => null);
            if (logChannel) {
            ...
            console.error('Erro ao enviar log de chamada:', err);
        }
    });
}
