...
module.exports = async (client) => {
    if (!client.user) {
        console.log('⚠️ Client ainda nao pronto, a aguardar ready event...');
        return;
    }
    ...
    console.log("🛡️ A inicializar sistema de verificacao...");
    try {
    ...
    try {
        const logChannel = await client.channels.fetch(config.LOGS_CHANNEL_ID).catch(() => null);
        if (logChannel) {
        ...
        console.error("Erro ao enviar log de inicializacao no Discord.", err.message);
    }
};
