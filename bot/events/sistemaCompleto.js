...
const CANAL_VOZ_ID = config.CANAL_VOZ_ID;
const CANAL_TICKET_ID = config.CANAL_TICKET_ID;
const CANAL_FORMULARIO_ID = config.CANAL_FORMULARIO_ID;
const CATEGORIA_TICKETS_ID = config.CATEGORIA_TICKETS_ID;
...
async function registrarComandosVoz(client) {
    try {
        ...
        const comandoAudio = new SlashCommandBuilder().setName('audio').setDescription('🎵 Controlar música no canal de voz')
    ...
    if (commandName === 'reiniciar') {
        try {
            ...
            if (audioPath) {
                ...
                if (sucesso) await interaction.editReply({ content: '🔄 Áudio reiniciado!' });
                else await interaction.editReply({ content: '❌ Erro ao reiniciar áudio.' });
            } else {
...
async function handleAudioCommand(interaction) {
    ...
    switch (subcommand) {
        case 'play':
            ...
            isPlaying = false;
            const sucesso = await tocarAudioLoopInfinito(audioPath);
            if (sucesso) await interaction.editReply({ content: '🎵 Música em loop infinito!' });
            else await interaction.editReply({ content: '❌ Erro ao tocar ficheiro. Verifica se o ficheiro existe na pasta /audio/' });
            break;
        case 'stop':
            if (pararAudio()) await interaction.editReply({ content: '🛑 Áudio parado' });
            else await interaction.editReply({ content: '❌ Nenhum áudio a tocar' });
            break;
        case 'volume':
            const nivel = interaction.options.getInteger('nivel');
            if (ajustarVolume(nivel)) await interaction.editReply({ content: `🔊 Volume ajustado para ${nivel}%` });
            else await interaction.editReply({ content: '❌ Não foi possível ajustar volume' });
            break;
    }
    return true;
}
...
async function handleFormBug(interaction) {
    ...
    const input1 = new TextInputBuilder().setCustomId('descricao_bug').setLabel('Descrição do Bug').setPlaceholder('Descreve o bug detalhadamente...').setStyle(TextInputStyle.Paragraph).setRequired(true).setMaxLength(1000);
...
async function handleFormIdeia(interaction) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    const modal = new ModalBuilder().setCustomId('modal_ideia').setTitle('💡 Sugestão');
    const input = new TextInputBuilder().setCustomId('descricao_ideia').setLabel('A tua ideia').setPlaceholder('Descreve a tua sugestão...').setStyle(TextInputStyle.Paragraph).setRequired(true).setMaxLength(2000);
...
async function handleFormAvaliar(interaction) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    const embed = new EmbedBuilder().setTitle('⭐ Avalia o Jordan Shop Bot').setDescription('Quantas estrelas dás ao nosso serviço (bot)?').setColor('#FFD700');
    const row = new ActionRowBuilder().addComponents(
...
async function handleAvaliacaoEstrelas(interaction, estrelas) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    const modal = new ModalBuilder().setCustomId(`modal_avaliacao_${estrelas}`).setTitle(`⭐ Avaliação: ${estrelas} Estrelas`);
    const input = new TextInputBuilder().setCustomId('motivo_avaliacao').setLabel('Comentário (opcional)').setPlaceholder('Conta-nos o que gostaste ou como podemos melhorar...').setStyle(TextInputStyle.Paragraph).setRequired(false).setMaxLength(1000);
    modal.addComponents(new ActionRowBuilder().addComponents(input));
    await interaction.followUp({ content: 'Abre o modal acima!', flags: MessageFlags.Ephemeral });
    await interaction.showModal(modal);
}
...
async function handleModalSubmit(interaction) {
    ...
    if (customId === 'modal_bug') {
        ...
        if (logChannel) {
            const embed = new EmbedBuilder().setTitle('💡 Nova Sugestão').addFields(
                { name: 'Utilizador', value: `<@${user.id}>`, inline: true },
        ...
        if (logChannel) {
            const embed = new EmbedBuilder().setTitle('⭐ Nova Avaliação').addFields(
                { name: 'Utilizador', value: `<@${user.id}>`, inline: true },
