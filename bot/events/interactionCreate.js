...
// ========== FORMATAR PRODUTO ==========
function formatarProduto(tipoProd) {
    for (const menu of menus) {
        for (const opt of menu.options) {
            if (opt.value === tipoProd) {
                return {
                    nome: menu.title,
                    opcao: opt.label,
                    preco: extrairPreco(opt.description),
                    descricao: opt.description
                };
            }
        }
    }
    return { nome: tipoProd, opcao: tipoProd, preco: null, descricao: null };
}
...
function extrairPreco(texto) {
    ...
    const match2 = texto.match(/([\d,.]+)\s*[€$]/);
    if (match2) return match2[1].replace(',', '.');
    return null;
}

// ========== STAFF SEM DUPLICADOS ==========
function getStaffOnline(guild) {
    const staffMap = new Map();
    guild.members.cache.forEach(m => {
        if (m.roles.cache.some(r => config.STAFF_ROLES.includes(r.id)) && !m.user.bot) {
            if (!staffMap.has(m.id)) staffMap.set(m.id, m);
        }
    });
    return Array.from(staffMap.values()).sort((a, b) => {
        return (b.roles.highest.position - a.roles.highest.position) || a.displayName.localeCompare(b.displayName);
    });
}
        ...
        try {
            ...
            if (interaction.isChatInputCommand()) {
                ...
                if (interaction.commandName === "adicionar") {
                    const cmd = require("../commands/adicionar");
                    return await cmd.execute(interaction);
                }
                if (interaction.commandName === "carrinho") {
                    const cmd = require("../commands/carrinho");
                    return await cmd.execute(interaction);
                }
            }
            ...
            if (interaction.isButton() && cid?.startsWith("recusar_termos_")) {
                ...
                if (canalLogs && !isDuplicateTicketLog(user.id, `recusar_${tipoRec}`, channel.id)) {
                    let tagFinal = "# ⭐ produto";
                    const tipoLower = tipoRec.toLowerCase();
                    ...
                    const embedLog = new EmbedBuilder()
                        .setColor(0xFF0000)
                        .setDescription(`❌ <@${user.id}> (${user.username}) **nao aceitou** os termos para abrir ticket de: **${tipoRec}** ${tagFinal}`)
                        .setTimestamp();
            ...
            if (interaction.isButton() && cid?.startsWith("aceitar_termos_")) {
                ...
                if (!isDuplicateTicketLog(user.id, `aceitar_${tipoAceito}`, channel.id)) {
                    const canalLogsTicket = guild.channels.cache.get(config.TICKET_LOGS_ID);
                    let tagFinal = "# ⭐ produto";
            ...
            if (interaction.isStringSelectMenu() && cid?.startsWith("pagamento_")) {
                ...
                const metodoFormatado = metodosFormatados[metodo] || metodo;
                const produtoInfo = formatarProduto(tipoProd);
                ...
                let descricao = `📦 **Produto:** ${produtoInfo.nome}\n`;
                if (produtoInfo.opcao && produtoInfo.opcao !== tipoProd) {
                    descricao += `   └─ ${produtoInfo.opcao}\n`;
                }
                descricao += `🆔 **Canal:** <#${ticket.id}>\n`;
                descricao += `💳 **Metodo:** ${emoji} ${metodoFormatado}\n`;
                if (produtoInfo.preco) {
                    descricao += `💰 **Valor:** €${produtoInfo.preco}\n`;
                }
                descricao += `👤 **Staff:** ${config.EMOJI_THREEDOTS} Aguardando...`;
                ...
                const rowGo = new ActionRowBuilder().addComponents(
                    new ButtonBuilder().setLabel("Ir para o Ticket/Pedido").setStyle(ButtonStyle.Link).setURL(`https://discord.com/channels/${guild.id}/${ticket.id}`)
                );
            ...
            if (cid === "claim_ticket") {
                ...
                const metFormatado = metodosFormatados[met] || met;
                const produtoInfo = formatarProduto(pdr);

                let descricao = `📦 **Produto:** ${produtoInfo.nome}\n`;
                if (produtoInfo.opcao && produtoInfo.opcao !== pdr) {
                    descricao += `   └─ ${produtoInfo.opcao}\n`;
                }
                descricao += `🆔 **Canal:** <#${channel.id}>\n`;
                descricao += `💳 **Metodo:** ${emj} ${metFormatado}\n`;
                if (produtoInfo.preco) {
                    descricao += `💰 **Valor:** €${produtoInfo.preco}\n`;
                }
                descricao += `🛡️ **Ticket Reivindicado**\n👤 **Staff:** <@${user.id}>`;
                ...
                if (canalLogsTicket) {
                    const embedLog = new EmbedBuilder()
                        .setColor(0x57f287)
                        .setDescription(`🛡️ <@${user.id}> (${user.username}) reivindicou o ticket <#${channel.id}> (\`${channel.name}\`)`)
                        .setTimestamp();
                    canalLogsTicket.send({ embeds: [embedLog] }).catch(() => {});
                }
            ...
            if (cid === "call_staff_list") {
                ...
                const staffOnline = getStaffOnline(guild);
                if (staffOnline.length === 0) return await interaction.reply({ content: "Sem Staff online.", flags: [64] });
            ...
            if (cid === "notify_staff_id") {
                ...
                await channel.send(`📢 <@${target.id}> (${target.user.username}), foste solicitado aqui por <@${user.id}> (${user.username})!`);
                const canalLogsTicket = guild.channels.cache.get(config.TICKET_LOGS_ID);
                if (canalLogsTicket) {
                    const embedLog = new EmbedBuilder()
                        .setColor(0xf1c40f)
                        .setDescription(`📢 <@${user.id}> (${user.username}) chamou <@${target.id}> (${target.user.username}) no ticket <#${channel.id}> (\`${channel.name}\`)`)
                        .setTimestamp();
                    canalLogsTicket.send({ embeds: [embedLog] }).catch(() => {});
                }
                return await interaction.update({ content: `📢 <@${target.id}> (${target.user.username}), foste solicitado aqui por **${user.username}**!`, components: [] });
            }
